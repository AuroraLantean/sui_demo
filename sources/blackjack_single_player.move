module package_addr::single_player_blackjack {
  ...
  public entry fun initialize_house_data(
		...
		assert!(coin::value(&gasCoinId) > 0, EInsufficientBalance);
  ...
  public entry fun top_up(house_data: &mut HouseData, gasCoinId: Coin<SUI>, _: &mut TxContext) {
		let balance = gasCoinId.into_balance();
  ...
  /// @param gasCoinId: The coin object that will be used to take the player's stake
  /// @param house_data: The HouseData object
  public entry fun place_bet_and_create_game(
		...
		assert!(gasCoinId.value() == STAKE, EInvalidPlayerBetAmount);
		...
		initial_randomness.append(user_counter.get_vrf_input_and_increment());
  ...
  fun get_card_sum(cards: &vector<u8>): u8 {
		...
		while (i < n) {
			let card = cards[i as u64];

			let mut value = (card % 13) + 1 ;  // this constraints index to the space [1-13]
			// 1 = Ace,2 = 2,  3 = 3, ...
			// 10 = 10
			// 11 = J (value 10)
			// 12 = Q (value 10)
